...
function toPlatformEphermalKey(keyAsObject) {
  if (!keyAsObject) return keyAsObject
  return Platform.select({
    ios: keyAsObject,
    android: JSON.stringify(keyAsObject),
  })
}
...
class Stripe {
  ...
  paymentRequestWithPaymentMethods = (options = {}) => {
    checkInit(this)
    return StripeModule.paymentRequestWithPaymentMethods({
      ...options,
      ephemeralKey: toPlatformEphermalKey(options.ephemeralKey),
      theme: processTheme(options.theme),
    })
  }

  completePaymentRequestWithPaymentMethods = (ephemeralKey) => {
    checkInit(this)
    return StripeModule.completePaymentRequestWithPaymentMethods(
